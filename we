...
def poisson_over(lmbda, threshold):
    cumulative = 0
    for k in range(threshold + 1):
        cumulative += (lmbda ** k * math.exp(-lmbda)) / factorial(k)
    return 1 - cumulative
...
home_strength = st.number_input("Home Strength", value=1.0, step=0.1)
away_strength = st.number_input("Away Strength", value=1.0, step=0.1)

avg_goals = st.number_input("Expected Average Goals", value=2.5, step=0.1)

st.subheader("Bookmaker Odds (Optional)")
odds_1 = st.number_input("Odds 1", value=0.0, step=0.01)
odds_x = st.number_input("Odds X", value=0.0, step=0.01)
odds_2 = st.number_input("Odds 2", value=0.0, step=0.01)


if st.button("Calculate Probabilities"):

    if home_strength + away_strength == 0:
        st.error("Strength values cannot both be zero.")

    else:
        raw = {
            "1": home_strength / (home_strength + away_strength),
            "2": away_strength / (home_strength + away_strength),
        }
